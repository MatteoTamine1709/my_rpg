...
void check_tile_mg(game_t *game, sfVector2i ij, int *val, sfVector2i *size)
{
    sfVector2i pos = (sfVector2i) {(int) (game->player->pos.x / 120),
        (int) ((game->player->pos.y + (30 * game->player->size.y)) / 120)};
    int index = game->mg_map->map[pos.x -
...
void check_other_tile_mg(player_t *player, game_t *game,
    int *val, sfVector2i *size)
{
    ...
    for (int i = pos.y; i >= 0; i--)
        for (int j = pos.x; j >= 0; j--)
            check_tile_mg(game, (sfVector2i){i, j}, val, size);
}

int check_pixel_mg(game_t *game, player_t *player, int val, sfVector2i size)
{
    sfImage *image = NULL;
...
int hitbox_mg(game_t *game, player_t *player)
{
    ...
    if (game->mg_map == NULL)
        return (0);
    val = game->mg_map->map[(int)(player->pos.x / 120)]
        [(int)((player->pos.y + (30 * game->player->size.y)) / 120)];
    if (val == -1)
        check_other_tile_mg(player, game, &val, &size);
    return (check_pixel_mg(game, player, val, size));
}